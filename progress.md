 ...
 * 修改时间：2026-09-17
 * AI模型：GLM (CodeBuddy)
 * 修改内容：[地图格式升级：1. 新增顶部“项目规模速览”小节（端级量级 + ASCII 边界图，不写精确行数）；2. 目录职责表升级为“路径 | 职责 | 何时去”三列；3. 精简低价值条目后重回 150 行预算，清单声明 50 项]
 */
/**
 * [变更日志]
 * 修改时间：2026-09-17
 * AI模型：GLM (CodeBuddy)
 * 修改内容：[1. 按用户指令物理删除旧C端残留目录 toc/（403 事故死路径本体）；2. 待确认两项销号：ops.py 双 router 语义查明（admin_router=B端运营管理面，member_router=C端横幅只读面，另含全局审计/通知 helper）登记入地图职责表；docs/v1.5test/ 确认为垃圾清理残留空壳并删除；3. 地图清单同步缩减至 53 项，四层自检复跑全绿（134 行）]
 */
/**
 * [变更日志]
 * 修改时间：2026-09-17
 * AI模型：GLM (CodeBuddy)
 * 修改内容：[1. 建成 project-map.md 项目地图体系（三端职责/启动路由/部署三处/陷阱区/待确认区 + map-paths 机器可读清单块，131 行预算内）；2. 新增 scripts/test_project_map.py 四层自检（清单存在性/agent.md 引用/活跃区反向扫描/150 行预算），独立与 pytest 双模运行均绿灯；3. agent.md 接入地图体系五触点（启动触发条件/结构性变更同步/收口闸门/大版本归档）；4. 反向扫描首跑即捕获未登记残留目录 toc/（仅剩旧 dist，可清理待用户决策）与清单路径勘误（pages.json 实际在 toc-new/src/ 下）]
 */
/**
 * [变更日志]
 * 修改时间：2026-09-14
...
## 四、项目地图体系备忘 (Project Map Memo)

**本轮新增的文档基建，后续 Agent 必须知晓：**

1. **`project-map.md`（项目地图）**：全项目目录的物理布局 + 读取路由。定位代码前、疑惑目录用途时、结构性变更后必读。其 `map-paths` 清单块是自检脚本的唯一解析面——**增删移目录必须同步该清单块**。
2. **`scripts/test_project_map.py`（四层自检）**：独立运行 `python scripts/test_project_map.py` 或并入 pytest。结构性变更落地后顺手跑（5 秒）；收口时必跑，**红灯禁止签署冻结时间戳**。
3. **地图设计契约**（详见地图头部）：粒度锁目录级、机器事实源只路由不复制、只写现状不写规划、≤150 行、待确认禁止猜测。
4. **待确认销号清单**：~~① ops.py 双 router 职责细分；② docs/v1.5test/ 空目录用途~~ **已全部销号（2026-09-17）**：ops.py 查明为运营底座双角色拆分并登记入地图职责表；v1.5test/ 确认为垃圾清理残留空壳并已删除。
5. ~~**待用户决策**：根目录 `toc/` 是否物理删除~~ **已执行（2026-09-17 用户拍板删除）**：`toc/` 及其旧 `dist/` 已物理删除，地图清单与陷阱区同步更新。

---
*时间戳签名：2026-09-17 16:29:02 (GLM / CodeBuddy / toc残留删除、待确认两项销号，地图53项134行全绿)*

---

## 五、项目地图格式升级备忘 (Project Map Format v1.1)

**2026-09-17 对地图文档做的格式升级：**

1. **新增顶部“项目规模速览”小节**：端级量级描述（C 端 H5 / B 端管理台 / Python 后端）+ ASCII 端级边界图（真相源指向 `nginx.conf`），不写精确行数/文件数。
2. **目录职责表从两列升级为三列**：`路径 | 职责 | 何时去`，让 AI 直接拿到“什么时候该进这个目录”的判断。
3. **体积控制**：为保持 ≤150 行预算，删除了低价值的三级资源目录声明（assets/static/styles）和次要入口行（deps.py、tests），清单声明从 53 项精简到 50 项，最终定格在 **150 行**。

---
*时间戳签名：2026-09-17 17:04:15 (GLM / CodeBuddy / 地图格式升级：规模速览 + 三列职责表，150 行预算红线)*
