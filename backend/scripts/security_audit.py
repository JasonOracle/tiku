# -*- coding: utf-8 -*-
"""安全攻防验证脚本：45 项断言（身份越权15 + 数据完整性20 + 契约红线10）。
只读验证为主；除登录 token 外不引入新数据。
唯一例外：C07“未发布不可提交”需一个草稿任务做靶子——创建后立即删除并复核数量，
若删除失败则标 CRITICAL 并退出。
运行：python backend/scripts/security_audit.py（工作目录仓库根）
"""
import json
import sqlite3
import os
...
PASS_LINE = 50  # A(85+/38-)与B(50/12)同时满足7/3的阈值；60会误判B
CRITICAL = []
...
def check(cid, name, ok, detail=""):
    ...
    print(("PASS" if ok else "FAIL") + f" {cid} {name} | {detail}")
...
def main():
    god_tok, god_id = login(*GOD)
    a_adm, a_adm_id = login(*A_ADMIN)
    b_adm, b_adm_id = login(*B_ADMIN)
    a_toks, a_ids = [], []
    for ph in A_MEMBERS:
        t, i = login(ph, "123456")
        a_toks.append(t)
    ...
    a_task_ids = [x["id"] for x in d["data"]["items"]]  # [3,1]
    st, d = api("GET", "/api/v1/admin/tasks?page=1&size=100", b_adm, tenant=2)
    b_task_ids = [x["id"] for x in d["data"]["items"]]  # [4,2]
    ...
    st, d = api("GET", "/api/v1/member/task-records", b_toks[0], tenant=2)
    b_m1_rec0 = d["data"]["items"][0]["record_id"] if d["data"]["items"] else None
    b_task_other = [t for t in b_task_ids][0]
    a_task_pub = 1  # A手动卷（published）
    a_ai_task = [t for t in a_task_ids if t != 1][0]  # A的AI卷（动态解析，回填后会变）

    # ============ 1. 身份与越权（15项 A01-A15） ============
    # A01 成员调 admin 接口 403
    ...
    # 恢复自己被改的 display_name（写回 nickname，还原造数状态）
    ...
    check("D04", "B租户资源数=10", st == 200 and d["data"]["total"] == 10,
          f"total={(d.get('data') or {}).get('total')}")
    # D05/D06 记录数 10/10（DB只读 + 待核验池=0佐证）
    ...
    # D11/D12 每卷 short=1/5（两租户各2卷合并断言）
    def short_of(task_id, tok, tid):
        st, d = api("GET", f"/api/v1/admin/tasks/{task_id}", tok, tenant=tid)
        qs = d["data"]["questions"]
        return len(qs), sum(1 for q in qs if q.get("type") == "short")
    n1, s1 = short_of(1, a_adm, 1)
    n3, s3 = short_of(a_ai_task, a_adm, 1)
    check("D11", "A两卷各short=1/5", (n1, s1) == (5, 1) and (n3, s3) == (5, 1),
          f"t1={s1}/{n1} t{a_ai_task}={s3}/{n3}")
    b_t_manual = 2  # B手动卷（稳定）
    b_ai_task = [t for t in b_task_ids if t != 2][0]  # B的AI卷（动态解析）
    n2, s2 = short_of(b_t_manual, b_adm, 2)
    n4, s4 = short_of(b_ai_task, b_adm, 2)
    check("D12", "B两卷各short=1/5", (n2, s2) == (5, 1) and (n4, s4) == (5, 1),
          f"t{b_t_manual}={s2}/{n2} t{b_ai_task}={s4}/{n4}")
    # D13 成员资料六字段齐全（逐个 GET /auth/me，10成员）
    fields = ["nickname", "email", "occupation", "bio", "age", "gender"]
    bad = []
    for ph, tok in list(zip(A_MEMBERS, a_toks)) + list(zip(B_MEMBERS, b_toks)):
        st, d = api("GET", "/api/v1/auth/me", tok, tenant=1 if ph in A_MEMBERS else 2)
    ...
    # D14/D15 A会话2/消息8；D16 B会话1/消息2（DB只读+API自查佐证）
    ...
    # D19 通知存在且tenant正确（通知按 user_id 过滤：成员只能见自己的；
    # 故断言结构：能查到自己的、总数一致子集、无串租户、无发给非本租户用户）
    st, d = api("GET", "/api/v1/admin/notifications?page=1&size=50", a_toks[0], tenant=1)
    n_tot = (d.get("data") or {}).get("total", -1) if st == 200 else -1
    n_db = db_all("select count(*) c from notifications where tenant_id=1")[0]["c"]
    n_bad = db_all("select count(*) c from notifications where tenant_id not in (1,2)")[0]["c"]
    n_cross = db_all("select count(*) c from notifications where tenant_id=1 and user_id not in "
                     "(select user_id from sys_tenant_user where tenant_id=1)")[0]["c"]
    own_ids = [(d.get("data") or {}).get("items", [])] if st == 200 else []
    m1_wrong = db_all("select count(*) c from notifications where user_id="
    ...
    a_db = db_all("select count(*) c from audit_logs where tenant_id=1")[0]["c"]
    ...
    st, b = api("POST", "/api/v1/member/task-records/submit", a_toks[0],
                body={"task_id": a_task_pub, "time_spent": 60,
                      "answers": [{"resource_id": 1, "answer": ["A"]}]}, tenant=1)
    check("C02", "重复提交400", st == 400, f"status={st} {snippet(b)}")
    # C03 缺头：普通用户400 / 上帝403
    ...
    # C04 非法 gender/age 400（且资料未被污染）
    st_g, b_g = api("PUT", "/api/v1/auth/profile", a_toks[2],
                    body={"gender": "alien"}, tenant=1)
    st_a, b_a = api("PUT", "/api/v1/auth/profile", a_toks[2],
    ...
    # C06 AI落库 ai_rag_sources 非空（查两租户AI卷任务+其资源）
    st, d = api("GET", f"/api/v1/admin/tasks/{a_ai_task}", a_adm, tenant=1)
    t_src = d["data"].get("ai_rag_sources") if st == 200 else None
    qs = d["data"].get("questions", []) if st == 200 else []
    b_ai_task2 = [t for t in b_task_ids if t != b_t_manual][0]  # B的AI卷（动态解析）
    st_b, d_b = api("GET", f"/api/v1/admin/tasks/{b_ai_task2}", b_adm, tenant=2)
    ...
    check("C06", "AI落库ai_rag_sources非空", ok,
          f"A_task_src={t_src} B_task_src={t_src_b} q_src_empty={sum(1 for q in qs if not q.get('ai_rag_sources'))}/{len(qs)}")
    # C07 未发布不可提交（草稿靶子，用后即删）
    before_tasks = db_all("select count(*) c from tasks where tenant_id=1")[0]["c"]
    draft_id = None
    try:
        st, d = api("POST", "/api/v1/admin/tasks", a_adm,
                    body={"title": "SEC-AUDIT-TMP-DRAFT", "resource_ids": [1],
        ...
        if draft_id:
            ...
            after_tasks = db_all("select count(*) c from tasks where tenant_id=1")[0]["c"]
    ...
    # C09 member任务详情不泄露 correct_answer
    st, d = api("GET", f"/api/v1/member/tasks/{a_task_pub}", a_toks[0], tenant=1)
    ...
    # C10 删除账号攻击：DELETE/members/{id}应404/405（无此能力）+ PUT他人profile只改自己
    st, b = api("DELETE", f"/api/v1/admin/members/{a_ids[1]}", a_toks[0], tenant=1)
    no_del = st in (404, 405)
    if st in (200, 201, 204):
        CRITICAL.append(f"DELETE members意外成功 status={st}——疑似真实删除能力！")
    # PUT他人profile：带他人id字段只改自己，他人数据不变
    ...
    # 清理自己可能被改的 display_name
    api("PUT", "/api/v1/auth/profile", a_toks[0],
        body={"display_name": (me1["data"].get("display_name") if 'me1' in locals() else "A成员01")}, tenant=1)
    ...
    print(f"\n==== SUMMARY PASS {npass}/{len(RESULTS)} ====")
    for r in RESULTS:
        if not r["pass"]:
