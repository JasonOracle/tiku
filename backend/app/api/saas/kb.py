...
@router.get("/metrics")
def get_kb_metrics(ctx: dict = Depends(require_member), db: Session = Depends(get_db)):
...
def delete_doc(doc_id: int, ctx: dict = Depends(require_member), db: Session = Depends(get_db)):
    """物理删除知识库文档及关联切片。"""
    tid = ctx["tenant_id"]
    uid = ctx["user"].id
    role = ctx["role"]
...
@router.post("/query")
def test_kb_query(payload: Dict[str, Any], ctx: dict = Depends(require_member),
