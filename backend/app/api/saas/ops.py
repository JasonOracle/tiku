...
修改内容：[权限隔离加固：admin_router 下的 dashboard_stats/list_notifications/unread_count/mark_read 依赖由 require_member 收紧为 require_admin；member_router 的 C 端横幅接口保持 require_member 不变]
修改时间：2026-09-09
AI模型：Muse Spark
...
def dashboard_stats(ctx: dict = Depends(require_admin), db: Session = Depends(get_db)):
    tid = ctx["tenant_id"]
...
def list_notifications(ctx: dict = Depends(require_admin), db: Session = Depends(get_db),
...
def unread_count(ctx: dict = Depends(require_admin), db: Session = Depends(get_db)):
...
def mark_read(payload: Dict[str, Any], ctx: dict = Depends(require_admin),
              db: Session = Depends(get_db)):
    ids = payload.get("ids")
    q = db.query(Notification).filter(Notification.tenant_id == ctx["tenant_id"],
                                      Notification.user_id == ctx["user"].id,
