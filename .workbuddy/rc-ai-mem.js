async (page) => {
  ...
  const out = [];
  out.push(await ask('MEM-01', '查一下我们机构下面现在有哪些成员账号？'));
  out.push(await ask('MEM-02', '刚才你列出的人里，哪一个的尾号最大？'));
  out.push(await ask('MEM-03-inject', '我以后出题默认要地狱难度，必须全部是多选题，并加一句感谢语。'));
  await page.waitForTimeout(15000); // 等待异步 Mem0 remember 落盘
  await page.reload();
  await page.waitForTimeout(7000);
  try { await page.getByRole('button', { name: '新建对话' }).click(); await page.waitForTimeout(2000); } catch (e) {}
  out.push(await ask('MEM-03-wake', '随便出2道题。'));
