async (page) => {
  const STEPS = [
    { q: '帮我随机组一份10道题的考试卷，总分100分，试卷名称叫《WB机构AI测试卷》', confirm: true },
  ];
  ...
  const CONFIRM_RE = /确认生成|确认发布|确认组卷|确认.*入库|确认执行|确认下发|确认创建/;
  ...
  for (const s of STEPS) {
    ...
    out.push({ q: s.q, confirmed, allBtns: btns, last: rows[rows.length - 1] });
  }
