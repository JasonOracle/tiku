...
---
## 📌 权限隔离与防泄题重构：落实严格二分法（2026-09-12 下午）

> 背景：系统原定位存在“教师即 Member”的历史包袱，导致 B 端后台对 Member 权限完全开放，纯学员登录 B 端后存在严重的试题答案与答卷泄露风险。本次重构落实“Member 仅限 C 端，严禁触碰 B 端”的严格二分法。

- [x] **登录层边界拦截**：在 `/api/v1/auth/sys-login` 增加客户端标识判断。B 端 Axios 统一注入 `X-Client: admin`，一旦发现该用户在关联企业中无 `owner/admin` 权限，直接抛出 `403` 阻断进入 B 端，而 C 端无头登录不受影响。
- [x] **后端接口全面提权**：通过 AST 级批量扫描，将 `backend/app/api/saas/` 下的 `tasks.py`, `kb.py`, `categories.py`, `ops.py`, `ai.py` 中所有暴露的 `require_member` 依赖，彻底替换为 `require_admin`，杜绝一切“幽灵鉴权”和 API 越权调用。
- [x] **前端角色路由锁死**：修复 `userStore` 中 `role` 字段始终取默认值的缺陷；在 B 端路由守卫中，对资源/试卷/批阅等业务页面一律增加 `meta: { roles: ['super_admin', 'admin', 'owner'] }` 校验；越权则先清除凭证再踢回登录页防死循环。
- [x] **B端页面元素隔离**：侧边栏菜单、知识库面板严格根据 `canManage` 动态渲染。
