...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│   v1.0 MVP   │ ──> │v1.2 AI-Native│ ──> │v1.3 RAG&治理 │ ──> │v1.4 SaaS架构 │ ──> │v1.5 Geek App │
│基础客观题闭环│     │简答填空/预批改│     │私有文库/状态机│    │多租户/对话卡片│    │Uni-app移动重塑│
└──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘
  已归档 (v1.0)        已归档 (v1.2)        已归档 (v1.3)       已归档 (v1.4-final)    当前正在推进中...
```

- **v1.0 (MVP 基础闭环)**：
  - 跑通基础单选、多选、判断客观题的录入、组卷、PC/H5 考试答题与自动秒级算分全链路。
- **v1.2 (AI-Native 能力跃升)**：
  - 引入填空题与主观简答题引擎；
  - 接入大模型实现主观题 AI 全托管/预批改阅卷；引入试卷按出题人隔离、成员额度管理与双域审计留痕。
- **v1.3 (私有知识库 RAG 命题与状态机治理)**：
  - **私有知识库与 RAG 命题闭环**：接入私有文档切片检索与向量召回，AI 出题自动标注原文切片依据，支持侧边抽屉溯源高亮；
  - **严苛的数据资产守卫**：确立试卷 `draft → published → archived` 状态机流转（归档终态不可逆锁定），分类与题目建立删除引用防护墙；
  - **服务端权威防作弊**：开考入口锁定服务端开考时刻，杜绝客户端修改本地时钟作弊。
