    ...
    <div class="tool-card__body">
      ...
      <div v-if="toolName === 'create_exam_draft'" class="exam-draft-card">
        ...
        <el-collapse v-if="questions.length > 0">
          ...
          >
            ...
            <div class="draft-row" style="margin-top: 6px">
              <span class="draft-label">答案:</span>{{ draftPreview(q).answerText }}
            </div>
          </el-collapse-item>
        </el-collapse>
        ...
        <div v-if="!actionResolved" class="action-footer">
          <button class="btn-confirm-medium" @click="$emit('confirm', message)">确认创建试卷</button>
      ...
      <div v-else-if="toolName === 'create_question_draft'" class="draft-preview">
        <div class="draft-title">{{ draftPreview.arguments?.title || 'AI 批量出题' }}</div>
        <div class="draft-meta">
          <el-tag size="small" type="primary">{{ draftPreview.typeLabel }}</el-tag>
          <span>难度 {{ draftPreview.difficulty }} · {{ draftPreview.score }} 分</span>
        </div>
        <div v-if="draftPreview.options.length" class="draft-opts">
          <div
            v-for="(opt, oi) in draftPreview.options"
            :key="oi"
            class="draft-opt"
            :class="{ correct: opt.correct }"
          >
        ...
        <div class="draft-row">
          <span class="draft-label">正确答案:</span>{{ draftPreview.answerText }}
        </div>
        <div v-if="draftPreview.explanation" class="draft-row">
          <span class="draft-label">解析:</span>{{ draftPreview.explanation }}
        </div>
      </div>
      ...
      <div v-else-if="toolName === 'delete_exam'" class="exam-draft-card" style="border-color: #fed7aa; background: #fffaf5;">
        ...
        <div style="font-size: 13px; color: #475569; margin-bottom: 8px;">
          您正在通过 AI 助理申请物理删除试卷
          <span v-if="arguments?.keyword">（匹配关键词<strong>{{ arguments.keyword }}</strong>）</span>
          <span v-else-if="arguments?.exam_id">（指定试卷 ID<strong>{{ arguments.exam_id }}</strong>）</span>
        ...
        <div v-if="!actionResolved" class="action-footer">
          <button class="btn-confirm" @click="$emit('confirm', message)">确认删除试卷</button>
          <button class="btn-cancel" @click="$emit('cancel', message)">取消</button>
        </div>
      </div>
      ...
      <div v-if="!actionResolved && toolName !== 'create_exam_draft'" class="action-footer">
        <button :class="toolName === 'delete_exam' ? 'btn-confirm' : 'btn-confirm-medium'" @click="$emit('confirm', message)">
          {{ toolName === 'delete_exam' ? '确认删除试卷' : '确认执行' }}
        </button>
        <button class="btn-cancel" @click="$emit('cancel', message)">取消</button>
      </div>
    </div>
...
const hasShortQuestion = computed(() =>
  questions.value.some((q: any) => q?.type === 'short')
);
