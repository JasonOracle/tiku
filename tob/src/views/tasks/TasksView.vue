...
<template>
  <div class="page-card">
    ...
    <el-table :data="exams" v-loading="loading" stripe style="width: 100%; margin-top: 16px">
      ...
      <el-table-column label="考试时间" width="110">
        <template #default="{ row }">
          ...
          <el-tag v-else type="info" size="small" effect="plain">不限时</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="330" fixed="right">
        <template #default="{ row }">
          <el-button
            ...
            type="warning"
            size="small"
            class="manual-grade-btn"
            @click="handleOpenGrading(row)"
          >
            人工批阅 ({{ row.pending_count }}份)
          </el-button>
          ...
          <el-button v-if="row.status === 'draft'" type="primary" text size="small" @click="openEditDialog(row)">编辑/编排</el-button>
    ...
    <!-- 数据看板 Dialog -->
    <el-dialog v-model="statsDialogVisible" title="已上架任务数据分析数据" width="700px">
      <div v-loading="statsLoading">
        ...
        <el-divider content-position="left"><strong>用户提交历史明细</strong></el-divider>
          ...
          <el-col :span="8">
            <el-form-item label="随机排列">
              <el-tag :type="viewData?.is_random ? 'warning' : 'info'">
                {{ viewData?.is_random ? '随机条目顺序' : '固定顺序' }}
              </el-tag>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            ...
            <template #default="{ row }">
              <div class="q-detail-expand">
                ...
                <div class="expand-item" v-if="row.explanation">
                  <span class="expand-label">条目解析：</span>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="880px" top="30px" destroy-on-close>
      <el-form :model="form" label-width="100px">
        ...
        <el-row v-if="hasShortQuestion" :gutter="16">
          <el-col :span="24">
            <el-form-item label="AI 核验模式">
              <el-radio-group v-model="form.grading_mode">
                <el-radio label="manual">人工全权核验</el-radio>
                <el-radio label="ai_pre">AI 辅助预审</el-radio>
                <el-radio label="ai_auto">AI 自动托管</el-radio>
              </el-radio-group>
              <div class="score-calc-tip" style="margin-top: 6px">
                检测到<strong>简答题</strong>才显示此选项：人工全权不提交 AI 评分建议，提交后等待老师批改；
          ...
          <el-col :span="24">
            <el-form-item label="及格比例">
              ...
              <div class="score-calc-tip">
                动态计算任务总分：<strong style="color: #0284c7; font-size: 15px">{{ computedTotalScore }}</strong> 分 ➔ 
        ...
        <el-divider content-position="left"><strong>试卷已添加题目（支持拖拽上下排序与多选批量删除）</strong></el-divider>

        <div class="selected-box">
          <div class="box-header">
            <span>已添加题目 (共 <strong style="color: #0284c7">{{ selectedQuestions.length }}</strong> 题)</span>
...
import { Plus, Delete, DataAnalysis, View, Search, RefreshRight } from '@element-plus/icons-vue';
...
const handleStatusChange = (row: any, val: boolean) => {
  ...
  if (!val) {
    ElMessageBox.confirm('下架后任务将停止对成员开放（无成员作答时可重新编辑、上架或彻底删除；已有作答成绩则保留归档）。确定要下架吗？', '下架确认', {
      type: 'warning',
      confirmButtonText: '确认下架',
      cancelButtonText: '取消'
    }).then(async () => {
      await request.put(`/api/v1/admin/tasks/${row.id}/status?status=${newStatus}`);
      ElMessage.success('任务已成功下架');
      loadExams();
    });
  } else {
    request.put(`/api/v1/admin/tasks/${row.id}/status?status=${newStatus}`).then(() => {
      ElMessage.success('任务已成功上线发布');
      loadExams();
    });
  }
};
...
const openStatsDialog = async (row: any) => {
  ...
  try {
    const res: any = await request.get(`/api/v1/admin/tasks/${row.id}/stats`);
    statsData.value = res;
  } finally {
